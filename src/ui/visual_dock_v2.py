...
class VisualDockV2(QWidget):
    ...
    def paintEvent(self, event):
        ...
        status_text = f"目标: {self.detected_title}"
        if self.is_docked:
            status_text = f" [已锁定] {self.detected_title}"
    ...
    def check_window_collision(self):
        ...
        try:
            ...
            if target_windows:
                ...
                self.detected_title = "未发现 oliver 窗口"
                if self.is_docked:
                    self.is_docked = False
                    self.current_color = self.default_color
                    self.update()
    ...
    def save_config(self):
        ...
        data = {
            "dock_rect": {"x": self.x(), "y": self.y(), "width": self.width(), "height": self.height()},
            "status": "docked" if self.is_docked else "floating",
            "last_target": self.detected_title
        }
